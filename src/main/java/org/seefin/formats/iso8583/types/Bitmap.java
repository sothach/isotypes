...
public class Bitmap {
   ...
   */
  private BitSet[] bitmaps = new BitSet[]{new BitSet(), new BitSet(), new BitSet()};
   ...
   *                                  hexadecimal string
   */
  public static Bitmap parse(String hexBitmap) {
    if (hexBitmap == null) {
  ...
  public String asHex(Id map) {
    return BitsetUtil.bitset2Hex(bitmaps[map.index], 16);
  }

  public byte[] asBinary(Id map) {
   ...
   */
  public void setField(int fieldNb) {
   ...
   * @return
   */
  private Id getBitmapIdForField(int fieldNb) {
    if (fieldNb <= 64) {
      return Id.PRIMARY;
    }
  ...
  private int getFieldPosInBitmap(int fieldNb, Id bitmapId) {
    switch (bitmapId) {
      case PRIMARY:
        return fieldNb - 1;
      case SECONDARY:
   ...
   * @return
   */
  public boolean isFieldPresent(int fieldNb) {
    Id bitmapId = getBitmapIdForField(fieldNb);
    BitSet bitmap = bitmaps[bitmapId.index];
  ...
  public boolean isBitmapPresent(Id map) {
    if (map == Id.PRIMARY) {
      return true;
    }
  ...
  private void setBitmapPresent(Id bitmapId) {
